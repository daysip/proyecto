...
public class DetalleController implements Serializable {
    ...
    @EJB
    private bean.DetalleFacade ejbFacade;
    ...
    protected void setEmbeddableKeys() {
        selected.getDetallePK().setNumHab(selected.getHabitacion().getNumHab());
        selected.getDetallePK().setIdAdi(selected.getAdicionales().getIdAdi());
        selected.getDetallePK().setIdOrden(selected.getOrden().getIdOrden());
    }

    protected void initializeEmbeddableKey() {
        selected.setDetallePK(new entidades.DetallePK());
    }
    ...
    public Detalle getDetalle(entidades.DetallePK id) {
    ...
    public static class DetalleControllerConverter implements Converter {
        ...
        entidades.DetallePK getKey(String value) {
            entidades.DetallePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entidades.DetallePK();
            key.setIdOrden(values[0]);
            key.setNumHab(values[1]);
            key.setIdAdi(values[2]);
            return key;
        }

        String getStringKey(entidades.DetallePK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getIdOrden());
            sb.append(SEPARATOR);
            sb.append(value.getNumHab());
            sb.append(SEPARATOR);
            sb.append(value.getIdAdi());
            return sb.toString();
        }
